...
#include <iostream>
#include <string>
#include <sstream> //для перевода double в string

#include <math.h>
#include <cmath>
#include <iomanip> //для точного cout
#include <Windows.h>
...
void findFirstOperand(string*& expression_arr, short& index, short& count_open_brackets, short& count_close_brackets, short direction) // -1 - лево, +1 - право
{
    index += direction;
...
void findTwoOperands(string* expression_arr, short size, short index, short& start_remove_index, short& end_remove_index, double& left_operand, double& right_operand)
{
    ...
    short const_index = index;

    //ищем первый операнд слева
    ...
    //если количество открытых и закрытых скобок не совпадает, то сдвигаемся на разницу
    ...
    cout << "right_binary_operand = " << right_operand << endl;
...
void findOneLeftOperand(string* expression_arr, short size, short index, short& start_remove_index, double& left_operand)
{
    ...
    cout << "left_unary_operand = " << left_operand << endl;
}
...
void findOneRightOperand(string* expression_arr, short size, short index, short& end_remove_index, double& right_operand)
{
    ...
    cout << "right_unary_operand = " << right_operand << endl;
}
...
void result_replace(string*& expression_array1, short start_remove_index, short end_remove_index, double result, short& size1)
{
    ...
    cout << "end_remove_index = " << end_remove_index << endl;

    //до start_remove_index части одинаковые
...
double simple_arithmetic_operation(double a, double b, string operator1) //бинарные операции
{
...
double simple_arithmetic_operation(double a, string operator1) //унарные операции
{
...
{
    return (((expression[str_i] == '-' && ((str_i == 0) || (expression[str_i - 1] != ')' && !isDigit(expression[str_i - 1])))) //унарный минус

        || isDigit(expression[str_i]) || expression[str_i] == '.')); //или цифра, или точка
}
...
string* getExpressionArray(string expression, short expression_size, short& expression_array_size)
{
    ...
    expression_array_size = arr_i; //размер равен следующему элементу после последнего
...
string solveExpression(string expression, short expression_size)
{
    ...
    while (operators_quantity > 0) //пока нужно делать какие-то операции
    {
        printExpressionArray(expression_array, expression_array_size);
        printPriorityArray(operators_priority, operators_quantity);

        short max_priority_index = findMaxPriorityIndex(operators_priority, operators_quantity); //индекс максимального приоритета В СПИСКЕ ПРИОРИТЕТОВ
        short max_operator_index = findMaxOperatorIndex(expression_array, expression_array_size, max_priority_index); //индекс максимального приоритета В ВЫРАЖЕНИИ
    ...
    return expression_array[expression_array_size / 2]; //берёт из середины
}
